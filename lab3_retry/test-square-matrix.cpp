...
void TestOperationsSM(){
    ...
    SquareMatrix<int> sm_4 = sm_2 + sm_3;

    assert(sm_4.GetSize() == 25 && "Error: Failed to add Square Matrices");

    for(int i=0; i < 25; i++)
        assert(sm_4.Get(i) == (sm_2.Get(i) + sm_3.Get(i)) && "Error: Failed to add Square Matrices");
    ...
    SquareMatrix<int> sm_5 = sm_2 * 5;
    assert(sm_5.GetSize() == 25 && "Error: Failed to multiply a Square Matrix by a scalar");

    for(int i=0; i < 25; i++)
        assert(sm_5.Get(i) == (sm_2.Get(i) * 5) && "Error: Failed to multiply a Square Matrix by a scalar");
...
void TestConstructors(){
    ...
    SquareMatrix<int> sm_1(10);
    assert(sm_1.GetSize() == 100 && "Error: Failed to construct a Vector");
