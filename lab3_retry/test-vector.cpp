...
void TestOperationsV(){
    ...
    Vector<int> v_4 = v_2 + v_3;

    assert(v_4.GetLength() == 5 && "Error: Failed to add Vectors");

    for(int i=0; i < 5; i++)
        assert(v_4.Get(i) == (v_2.Get(i) + v_3.Get(i)) && "Error: Failed to add Vectors");


    Vector<int> v_5 = v_2 * 5;
    assert(v_5.GetLength() == 5 && "Error: Failed to a Vector by a scalar");

    for(int i=0; i < 5; i++)
        assert(v_5.Get(i) == (v_2.Get(i) * 5) && "Error: Failed to a Vector by a scalar");


    int result = v_2.ScalarMultiplication(v_3);
